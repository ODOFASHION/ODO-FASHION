...
(() => {
  ...
  function styles(){
    ...
    s.textContent=`
      ...
      .odo-live-tools{display:flex;gap:8px;margin-top:10px}.odo-live-tools button{border:1px solid rgba(200,165,96,.35);background:transparent;color:#e7dfd0;padding:8px 10px;font-size:.52rem;letter-spacing:.08em;cursor:pointer}.odo-live-tools .liked{color:#e5c982;border-color:#c8a560}.odo-live-tools .share{margin-left:auto}
  ...
  function toggleWishlist(p){
    if(wishlist.has(p.sku)){wishlist.delete(p.sku);toast(`${p.name} removed from wishlist`)}else{wishlist.add(p.sku);toast(`${p.name} saved`)}
  ...
  function dynamicCard(p){
    ...
    a.querySelector('.odo-live-add').onclick=()=>{
      const size=a.querySelector('.size-select')?.value||'ONE SIZE';
  ...
  async function loadAdminProducts(){
    try{
      ...
      return data||[];
    }catch(e){console.warn('ODO products:',e);return []}
  }

  function updateAccessoriesCategory(products){
    const cards=[...document.querySelectorAll('.category-card')]; const acc=cards.find(c=>/ACCESSORIES/i.test(c.textContent||''));
    if(!acc)return; const live=products.some(p=>p.category==='ACCESSORIES'); const b=acc.querySelector('b'); if(b)b.textContent=live?'SHOP NOW →':'COMING SOON';
    if(live) acc.classList.add('active');
  }

  function renderAdminAddedProducts(products){
    const staticSkus=new Set(staticProducts.map(p=>p.sku));
    const newStreet=products.filter(p=>staticSkus.has(String(p.sku))===false && (p.category==='STREETWEAR'||p.category==='OUTERWEAR'));
    const accessories=products.filter(p=>p.category==='ACCESSORIES' && staticSkus.has(String(p.sku))===false);
    const buckets=[];
    const makeSection=(id,label,title,list)=>{
      if(!list.length)return;
      let sec=document.getElementById(id); if(!sec){sec=document.createElement('section');sec.id=id;sec.className='section';const about=document.getElementById('about');about?.parentNode.insertBefore(sec,about)}
  ...
  async function boot(){
    styles(); mountWishlist(); decorateStaticProducts();
    const products=await loadAdminProducts();
    renderAdminAddedProducts(products);
    updateAccessoriesCategory(products);
    updateLikeButtons();
  }
